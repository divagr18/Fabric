...
All protocol code is cleanly encapsulated in [`app/src/webmcp/`](https://github.com/divagr18/fabric/tree/main/app/src/webmcp). Every core tool and every runtime-compiled tool reaches the standard WebMCP registration path in [`registry.ts`](https://github.com/divagr18/fabric/blob/main/app/src/webmcp/registry.ts):
...
document.modelContext.registerTool({
  ...
  execute: async (input) => {
    // Run the validated cross-device pipeline.
  },
}, { signal: controller.signal });
```

Fabric uses an `AbortController` for each registration. Aborting the previous registration and registering the new implementation is what makes hot-swapping possible without changing the interface the agent calls.
