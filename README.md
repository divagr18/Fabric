...
Fabric turns every browser you own — laptop, desktop, phone — into one runtime an agent can use through [WebMCP](https://github.com/webmachinelearning/webmcp). Devices join a room with a QR scan, each contributing only what its owner explicitly exposes. An external agent (ChatGPT) sees a small WebMCP tool surface and compiles the cross-device tools it needs at runtime; when the hardware changes, the tools hot-reload while their interface survives.
