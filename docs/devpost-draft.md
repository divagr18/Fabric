...
Cross-vendor: the same fabric page serving Chrome's built-in agent alongside ChatGPT — two vendors' agents sharing one consent membrane. Trace-driven tool suggestion (repeated primitive sequences offered as compiled tools). Persisted grant *policies* (capability re-consent in one tap when a device rejoins a remembered fabric).
